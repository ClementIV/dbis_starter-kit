...
abstract class BaseQyWechat extends BaseWechat
{
    ...
    const QY_MSG_DEPLIST = 'cgi-bin/department/list';
    ...
    {
        $ret           = $this->get(self::QY_MSG_DEPLIST,  ['access_token' => $this->getAccessToken()]);
        $this->lastRet = $ret;
        return isset($ret['errcode']) && $ret['errcode'] !== 0 ? false : $ret;
    }

    const QY_MSG_USERLIST = 'cgi-bin/user/list';
     ...
     * @return bool|mixed
     */
    public function listUser($department_id,$fetch_child=1,$status=1)
    {
        $ret           = $this->get(self::QY_MSG_USERLIST,
            [
                'access_token' => $this->getAccessToken(),
                'department_id' => $department_id,
                'fetch_child' => $fetch_child,
